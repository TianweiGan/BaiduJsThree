# 基于Baidu JSAPI Three的地球仪Demo

## 项目简介
本项目旨在演示如何在 React 环境下，结合 [@baidumap/mapv-three](https://lbsyun.baidu.com/faq/api?title=jsapithree) 和 Three.js，实现一个具备二三维地图切换能力，并能像地球仪一样自由旋转和缩放的地图应用。

## 效果截图
![Demo 效果图1](image/image1.png)
![Demo 效果图2](image/image2.png)
...
## 资源与构建配置
本项目采用 Webpack 进行打包，静态资源（如地图底图、自定义3D模型等）通过 CopyWebpackPlugin 自动拷贝到输出目录。
...
const Demo = () => {
    ...
    useEffect(() => {
        ...
        const engine = new mapvthree.Engine(ref.current, {
            map: {
                provider: new mapvthree.BaiduVectorTileProvider(), // 使用百度矢量瓦片作为基础地图数据
                projection: 'ECEF', // 设置为"地心固定坐标系"，为三维地球仪模式做准备
                center: [116, 39], // 地图的初始中心点经纬度
                heading: 40, // 相机初始航向角
                pitch: 0, // 相机初始俯仰角，0度表示水平视角，可调整以模拟地球仪的视角
                range: 2000, // 相机到中心点的初始距离（缩放级别），数字越小越放大
            },
            rendering: {
                enableAnimationLoop: true, // 启用动画循环以实现平滑渲染和交互
            },
        });
        // 组件卸载时释放资源
        return () => {
